...
const MigrationSticky: React.FC<MigrationStickyProps> = ({ step, handleClick }) => {
  ...
  const title: string = isStep1 ? t('Unstaking LP Tokens and WAG') : t('Stake in the new contract.')
  const subTitle: string = isStep1
    ? t('All the earned WAG will be harvested to your wallet upon unstake.')
  ...
  if (!account) {
    return (
      <Container>
        <TextGroup>
          <TextTitle bold>{t('WagFarm v2 Migration')}</TextTitle>
