...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69
  const [editingUnit, setEditingUnit] = useState<'WAG' | 'USD'>('WAG')
  const conversionUnit = editingUnit === 'WAG' ? 'USD' : 'WAG'
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === 'WAG' ? 'USD' : 'WAG'
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === 'WAG') {
      ...
      setValues({
        WAG: Number.isNaN(inputAsFloat) ? '' : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      })
    }
  }
