...
import { Pair, TokenAmount, CurrencyAmount, Trade } from '@wagyu-swap/sdk'
...
describe('Trade', () => {
  describe('#useAllCommonPairs', () => {
    ...
    it('should filter only exist Pair', () => {
      mockUsePairs.mockReturnValue([
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.wtlos, '1'), new TokenAmount(mainnetTokens.cake, '1')),
        ],
        [
          PairState.INVALID,
        ...
        [
          PairState.LOADING,
          new Pair(new TokenAmount(mainnetTokens.busd, '1'), new TokenAmount(mainnetTokens.wtlos, '1')),
        ],
        [PairState.EXISTS, null],
      ])

      const { result } = renderHook(() => {
        const pairs = Trades.useAllCommonPairs(mainnetTokens.wtlos, mainnetTokens.cake)
        return {
          pairs,
        }
      })
    ...
    it('should filter out duplicated Pair', () => {
      mockUsePairs.mockReturnValue([
        ...
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.cake, '1'), new TokenAmount(mainnetTokens.wtlos, '1')),
        ],
        [PairState.EXISTS, null],
      ])
    ...
    it('should get all pair combinations wtlos, cake', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wtlos, mainnetTokens.cake)
      })

      expect(mockUsePairs).toMatchSnapshot()
    })

    it('should get all pair combinations, wtlos, wtlos', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wtlos, mainnetTokens.wtlos)
      })
  ...
  describe('#useTradeExactIn/Out', () => {
    ...
    it('should call with maxHops 1 with singleHopOnly', () => {
      const allowPairs = [new Pair(new TokenAmount(mainnetTokens.wtlos, '1'), new TokenAmount(mainnetTokens.cake, '1'))]
