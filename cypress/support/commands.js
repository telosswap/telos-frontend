 ...
 * One test in swap.test.ts requires to have some VLX amount available to test swap confirmation modal
...
class CustomizedBridge extends Eip1193Bridge {
  ...
  async send(...args) {
    ...
    if (method === 'eth_chainId') {
      if (isCallbackForm) {
        return callback(null, { result: '0x6a' })
      }
      return Promise.resolve('0x6a')
    }
    try {
